...
def train(args, model, loader, optimizer, lambdaL1):
    model.train()

    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        pred, l1_loss = model(batch)
        y = torch.stack(batch['outcomes']).view(pred.shape).to(torch.float64)
        optimizer.zero_grad()
        loss = criterion(pred.double(), y) 
        print('loss is:', loss, 'l1_loss is: ', lambdaL1*l1_loss)
        loss += lambdaL1*l1_loss 
        loss.backward()
...
def main():
    ...
    parser.add_argument('--model_file', type=str, default = 'pretrain_model/mask_supervised', help='filename to read the model (if there is any)')
    parser.add_argument('--seed', type=int, default=123, help = "Seed for running experiments.")
    parser.add_argument('--lambdaL1', type=float, default=5, help = "lambda for l1 loss.")
    ...
    #set up model
    model = GNN_graphpred2(args.num_layer, args.emb_dim, ext_dim = extra_dim, out_dim = 1, JK = args.JK, drop_ratio = args.dropout_ratio, graph_pooling = args.graph_pooling)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        train_loss = train(args, model, train_loader, optimizer, args.lambdaL1)
    ...
    # Example usage:
    save_path = 'loss_graph_fullext.png'

    draw_loss_graph(loss_list, save_path)


    os.makedirs(f"result/finetune_seed{args.seed}" , exist_ok=True)
    filepath = f"result/finetune_seed{args.seed}"+"/"+"fullmodel_ext_result.pkl"
