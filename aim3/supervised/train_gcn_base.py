...
for i in range(len(data_cv)):
    ...
    ######2. create model
    model = GCN_base(nfeat = num_features,
                nhid = args.nhid,
                nclass=2,
    ...
    def compute_valid(loader):
        ...
        with torch.no_grad():
            for data in loader:
                edge_list, node_feature, subject_feature, outcome = data
                out = model(node_feature, edge_list)
    ...
    def train(epoch, train_loader, val_loader):
        ...
        for i, data in enumerate(train_loader):
            ...
            model.train()
            optimizer.zero_grad()
            output = model(node_feature, edge_list)
