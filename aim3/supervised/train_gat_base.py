...
parser.add_argument('--nheads', type=int, default=8, help='number of multi head')
...
data_cv, num_features, num_features_extra = load_data2(cont_mat_path, node_feature_path, subject_feature_path, seed, args.cv_size)

sv_path = "../results/gat_base/"
patience = 50
...
for i in range(len(data_cv)):
    ...
    ######2. create model
    model = GAT_base(nfeat = num_features,
                nhid = args.nhid,
                nclass=2,
    ...
    def compute_valid(loader):
        ...
        with torch.no_grad():
            for data in loader:
                edge_ls, node_feature, subject_feature, outcome = data
                
                out = model(node_feature, edge_ls)
    ...
    def train(epoch, train_loader, val_loader):
        ...
        for i, data in enumerate(train_loader):
            ...
            model.train()
            optimizer.zero_grad()
            output = model(node_feature, edge_ls)
            loss = F.nll_loss(output, outcome.long(), reduction="mean")
            loss.backward()
            optimizer.step()
