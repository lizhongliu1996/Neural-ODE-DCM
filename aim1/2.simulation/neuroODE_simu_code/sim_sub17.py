...
class RandomLinearODEF(LinearODEF):
    def __init__(self):
        super(RandomLinearODEF, self).__init__(torch.randn(10, 10)/5.)
...
def conduct_experiment(obs, ode_trained, n_steps, name, plot_freq=50, epoch=5):
    # Create data
    z0 = Variable(torch.Tensor([[-1.0, 0, 0.1, 0.1, 0.42, 0.05, -0.03, 0.02, 0.8, -0.4]]))
...
import scipy.io
mat = scipy.io.loadmat('sim/sim17.mat')
...
save("simu_results/sim17.npy", result)

# result_df = pd.DataFrame(result)
# result_df.columns = ['tp', 'acc', 'Euclidean_dist', 'spec_dist', 'edit', 'netsmile']
# result_df.to_csv(f"neuro_results/thre{thre}/sim_sub17.csv")
