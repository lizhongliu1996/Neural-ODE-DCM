...
#obs2 = torch.from_numpy(median)
#obs2 = np.reshape(obs2, (269,1,264))
...
for epoch in range(1, n_epochs + 1):
    train_losses, final_pars = conduct_experiment(obs, ode_trained, 3000, epoch = epoch)
...
plt.plot(x_ls, Loss_list, 'o-')
plt.xlabel('epochs')
plt.ylabel('Test loss')
plt.show()
plt.savefig("ncanda_loss.jpg")
# par_ls2 = []
# for epoch in range(1, n_epochs + 1):
#     train_losses, final_pars = conduct_experiment(obs2, ode_trained, 3000, epoch = epoch)
#     print('Median Train loss: {:.5f}'.format(np.mean(train_losses)))       
#     pars = final_pars[0].detach()
#     pars_ls2.append(pars)
#par2 = pars_ls[-1].numpy()

result_df = pd.DataFrame(par)
result_df.to_csv("ncanda_result.csv")
#result2.to_csv("ncanda_result2.csv")
